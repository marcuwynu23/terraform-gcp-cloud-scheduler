...
- `google_project_service.scheduler_api` – Enables Cloud Scheduler API
- `google_cloud_scheduler_job.job` – Cloud Scheduler cron job
...
3. **Add GitHub secrets**:
    ...
    | `TF_BUCKET_PREFIX` | Bucket prefix/path (e.g., `gcp-cloud-scheduler`) |

4. **Run the workflow**:
    - **Apply**: Go to Actions → **CD - GCP Cloud Scheduler (Apply)** → fill in all inputs
    - **Destroy**: Go to Actions → **CD - GCP Cloud Scheduler (Destroy)** → fill in essential inputs
